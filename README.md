# Device-Drivers
Projects and informations related to Device Drivers

## Commands
- make
- make clean
- insmod hello_world.ko
- dmesg
- rmmod hello_world
- lsmod (Shows all loaded drivers)

## info
- my_device will be in /dev
- my_dev will be in /proc/devices
- my_custome_driver will be in /proc/modules
- use cc test_chr_drv.c -o test_chr_drv
- sudo ./test_chr_drv
- container_of_macro is a proof of concept so build it normally with gcc command

## In-tree building
- If you want to add the linux module inside the linux kernel source tree and let the linux build system builds that. If you want to list your kernel module selection in kernel menuconfig,then create and use a Kconfig file.
### steps:
- create a folder in linux-<version>/drivers/char/my_c_dev
- copy main.c
- create Kconfig file and add the below entries:
  menu "my custom menu"
  config CUSTOM_HELLOWORLD
          tristate "helloworld module support"
          default n
  endmenu
- add the local Kconfig entry to upper(char directory) level Kconfig
- open Kconfig from char directory and go to the end, write source "drivers/char/my_c_dev/Kconfig"
- create a local Makefile
- go to "drivers/char/my_c_dev" and create Makefile add the content in step 6
- add obj-<config_item>+=<module>.o in the local Makefile
- obj-$(CONFIG_CUSTOM_HELLOWORLD) += main.o
- add the local makefile to higher level makefile
- got to "drivers/char" and edit Makefile, go to the end and add
- obj-y += my_c_dev/
- go to linux-<version> directory run the command
- make ARCH=arm menuconfig, it will open the menu
- go to device drivers->character devices->my custom menu->helloworld module support
- select * or M using space key and save
- open .config file to verify that CONFIG_CUSTOM_HELLOWORD contains selected value
- execute make ARCH=arm CROSS_COMPILE=arm-linux-gnueabihf- modules -j4, you can see the module has build and .ko file is generated.
- go to drivers/char/my_c_dev and run modinfo main.ko, you will find intree as Y

## Downloads
- git clone https://github.com/beagleboard/linux.git
