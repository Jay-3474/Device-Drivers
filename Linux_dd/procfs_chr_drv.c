...
#include <linux/proc_fs.h> // struct proc_ops, proc_create, proc_mkdir, remove_proc_entry

char chr_array[40] = "Walcome to the proc file system\n";
static int len = 1;
...
static int release_proc(struct inode *inode, struct file *fie);
...
static ssize_t write_proc(struct file *file, const char *buf, size_t len, loff_t *off)
{
	printk(KERN_INFO "Procfs file writting..\n");
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Jay");
MODULE_DESCRIPTION("My first procfs Device Driver");
