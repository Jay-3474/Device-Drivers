...
static struct kobj_attribute sysfs_attr = __ATTR(sysfs_val, 0660, sysfs_show, sysfs_store);
...
static ssize_t sysfs_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	printk(KERN_INFO "Reading in sysfs show func...\n");
	return sprintf(buf, "%d", sysfs_val);
}
static ssize_t sysfs_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	printk(KERN_INFO "Writing in sysfs store func...\n");
	sscanf(buf, "%u", &sysfs_val);
	return count;
}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Jay");
MODULE_DESCRIPTION("My first sysfs Device Driver");
