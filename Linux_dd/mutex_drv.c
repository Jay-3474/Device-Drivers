...
static int __init chr_driver_init(void)
{
	...
	chr_thread1 = kthread_run(thrd_func1, NULL, "mutex thread %d", 1);
	if(chr_thread1)
	{
	...
	{
		printk(KERN_ERR "unable to read the kernel thread 1...\n");
		goto r_device;
	}
	
	/*creating the second thread*/
	chr_thread2 = kthread_run(thrd_func2, NULL, "mutex thread %d", 2);
	if(chr_thread2)
	{
	...
	{
		printk(KERN_ERR "unable to read the kernel thread 2...\n");
		goto r_device;
	}
